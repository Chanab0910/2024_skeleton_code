...
class Puzzle():
    def __init__(self, *args):
        self.successful_wildcard = False
        if len(args) == 1:
            self.wildcard = True
            self.__Score = 0
    ...
    def AttemptPuzzle(self):
        ...
        while not Finished:
            ...
            if self.wildcard:
                wildcard = input('Do you want to use your only wildcard: ')
                if wildcard == 'y':
                    old_symbol = CurrentCell.GetSymbol()
                    self.ApplyWildcard(Row,Column,Symbol, old_symbol)
            if CurrentCell.CheckSymbolAllowed(Symbol) and not self.successful_wildcard:
    ...
    def ApplyWildcard(self,row,column,symbol,old_symbol):
        isblocked = False
        CurrentCell = self.__GetCell(row, column)
        ...
        if self.CheckforMatchWithPattern(row,column) == 10:
            self.__Score+=5
            self.wildcard = False
            self.successful_wildcard = True
        elif self.CheckforMatchWithPattern(row,column) == 0:
            print('Sorry, the wildcard does not work for this cell – you have no wildcards left')
            if isblocked:
                CurrentCell = BlockedCell()
            else:
                CurrentCell.ChangeSymbolInCell(old_symbol)
