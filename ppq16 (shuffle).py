...
class Puzzle():
    ...
    def AttemptPuzzle(self):
        ...
        while not Finished:
            ...
            Row = -1
            Valid = False
            shuffle = input("Do you want to shuffle your puzzle (y/n)? ")
            if shuffle.lower() == "y":
                P = Pattern('f','2')
                self.__Grid = P.ShuffleBlockedCells(self.__Grid)
            else:
                while not Valid:
                    try:
...
class Pattern():
    ...
    def ShuffleBlockedCells(self, grid):
        symbols = []
        coor = []
        for i, cell in enumerate(grid):
            if not cell.is_restricted():
                symbols.append(cell.GetSymbol())
                coor.append(i)
        print(coor)
        shuffle(coor)
        for i,cord in enumerate(coor):
            grid[cord].ChangeSymbolInCell(symbols[i])
        return grid
...
class Cell():
    ...
    def is_restricted(self):
        if len(self.__SymbolsNotAllowed) > 0 and self.__SymbolsNotAllowed != ' ':
            return True
        else:
            return False
