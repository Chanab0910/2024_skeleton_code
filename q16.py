...
def Main():
    ...
    while Again == "y":
        ...
        if len(Filename) > 0:  # looks to see if something was entered
            MyPuzzle = Puzzle(Filename + ".txt")  # If entered then it loads the puzzle written
        else:
            MyPuzzle = Puzzle(8, int(8 * 8 * 0.6)) # if nothing was written then it form a default puzzle that's 8x8
...
class Puzzle():
    ...
    def AttemptPuzzle(self):
        ...
        while not Finished:
            ...
            Row = -1
            Valid = False
            new_symbol = input('Do you want to add a new symbol to the game?(y/n): ').upper()
            if new_symbol == 'Y':
                self.new_symbol()
            while not Valid:
                try:
    ...
    def new_symbol(self):
        print('Input the 3x3 pattern you want recognised')
        print('If you want the square to be empty, enter *')
        ok = False
        while not ok:
            symbol = input('What symbol do you want to add?: ').upper()
            tl = input('What symbol do you want to be in the top left slot: ').upper()
            tm = input('What symbol do you want to be in the top middle slot: ').upper()
            tr = input('What symbol do you want to be in the top right slot: ').upper()
            ml = input('What symbol do you want to be in the middle left slot: ').upper()
            mm = input('What symbol do you want to be in the middle slot: ').upper()
            mr = input('What symbol do you want to be in the middle right slot: ').upper()
            bl = input('What symbol do you want to be in the bottom left slot: ').upper()
            bm = input('What symbol do you want to be in the bottom middle slot: ').upper()
            br = input('What symbol do you want to be in the bottom right slot: ').upper()
            wrong = input('Would you like to redo the entry?(y/n): ').upper()
            if wrong == 'N':
                ok =True

        pattern = Pattern(symbol, tl+tm+tr+mr+br+bm+bl+ml+mm)
        self.__AllowedPatterns.append(pattern)
        self.__AllowedSymbols.append(symbol)
        print(f'{symbol} has been added as a symbol')
